$(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.code == 200) {
                        ...
                        layer.msg(
                            data.msg || I18n.system_opt_del + I18n.system_fail);
                    }
                }
            });
        });
    ...
    $("#addModal .form input[name=role]").change(function () {
        var role = $(this).val();
        if (role == 1) {
            $("#addModal .form input[name=permission]").parents(
                '.form-group').hide();
        } else {
            $("#addModal .form input[name=permission]").parents(
                '.form-group').show();
    ...
    var addModalValidate = $("#addModal .form").validate({
        ...
        submitHandler: function (form) {
            ...
            var permissionArr = [];
            $("#addModal .form input[name=permission]:checked").each(
                function () {
    ...
    $("#updateModal .form input[name=role]").change(function () {
        var role = $(this).val();
        if (role == 1) {
            $("#updateModal .form input[name=permission]").parents(
                '.form-group').hide();
        } else {
            $("#updateModal .form input[name=permission]").parents(
                '.form-group').show();
    ...
    $("#user_list").on('click', '.update', function () {
        ...
        $("#updateModal .form input[name='role'][value='" + row.role
            + "']").click();
        var permissionArr = [];
        if (row.permission) {
    ...
    var updateModalValidate = $("#updateModal .form").validate({
        ...
        submitHandler: function (form) {
            ...
            var permissionArr = [];
            $("#updateModal .form input[name=permission]:checked").each(
                function () {
                    permissionArr.push($(this).val());
                });
            ...
            $.post(base_url + "/user/update", paramData,
