    ...
    CodeMirror.defineMode("javascript", function (config, parserConfig) {
        ...
        function tokenBase(stream, state) {
            ...
            if (ch == '"' || ch == "'") {
                ...
                return ret("=>", "operator");
            } else if (ch == "0" && stream.match(
                /^(?:x[\da-f]+|o[0-7]+|b[01]+)n?/i)) {
                return ret("number", "number");
            } else if (/\d/.test(ch)) {
        ...
        function tokenString(quote) {
            return function (stream, state) {
                ...
                if (jsonldMode && stream.peek() == "@" && stream.match(
                    isJsonldKeyword)) {
        ...
        function statement(type, value) {
            if (type == "var") {
                return cont(pushlex("vardef", value), vardef,
                    expect(";"), poplex);
            }
            if (type == "keyword a") {
            ...
            if (type == "keyword d") {
                return cx.stream.match(/^\s*$/, false)
                    ? cont() : cont(pushlex("stat"), maybeexpression,
                        expect(";"),
                        poplex);
            }
            if (type == "debugger") {
            ...
            if (type == "{") {
                return cont(pushlex("}"), pushblockcontext, block,
                    poplex, popcontext);
            }
            if (type == ";") {
                return cont();
            ...
            if (type == "for") {
                return cont(pushlex("form"), forspec, statement,
                    poplex);
            }
            ...
            if (type == "variable") {
                if (isTS && value == "declare") {
                    ...
                    || value == "type") && cx.stream.match(/^\s*\w/, false)) {
                    cx.marked = "keyword"
                    if (value == "enum") {
                        return cont(enumdef);
                    } else if (value == "type") {
            ...
            if (type == "catch") {
                return cont(pushlex("form"), pushcontext,
                    maybeCatchBinding, statement, poplex, popcontext);
            }
            if (type == "export") {
                return cont(pushlex("stat"), afterExport,
                    poplex);
            }
            if (type == "import") {
                return cont(pushlex("stat"), afterImport,
                    poplex);
            }
            if (type == "async") {
                return cont(statement)
        ...
        function expressionInner(type, value, noComma) {
            ...
            if (type == "[") {
                return cont(pushlex("]"), arrayLiteral, poplex,
                    maybeop);
            }
            if (type == "{") {
        ...
        function maybeoperatorNoComma(type, value, noComma) {
            ...
            if (type == "(") {
                return contCommasep(expressionNoComma, ")", "call",
                    me);
            }
            if (type == ".") {
        ...
        function forspec(type, value) {
            ...
            if (type == "(") {
                return cont(pushlex(")"), forspec1, expect(")"),
                    poplex);
            }
        }
        ...
        function afterExport(type, value) {
            ...
            if (type == "{") {
                return cont(commasep(exportField, "}"), maybeFrom,
                    expect(";"));
            }
        ...
        function enumdef() {
            return pass(pushlex("form"), pattern, expect("{"), pushlex("}"),
                commasep(enummember, "}"), poplex, poplex)
        }
        ...
        return {
            ...
            indent: function (state, textAfter) {
                ...
                if (statementIndent && lexical.type == ")" && lexical.prev.type
                    == "stat") {
                    lexical = lexical.prev;
                }
                ...
                if (type == "vardef") {
                    return lexical.indented + (state.lastType
                    == "operator" || state.lastType == "," ? lexical.info.length
                        + 1
                        : 0);
                    ...
                    return lexical.indented + (isContinuedStatement(state,
                    ...
                    return lexical.indented + (/^(?:case|default)\b/.test(
                        textAfter) ? indentUnit : 2 * indentUnit);
                } else if (lexical.align) {
                    return lexical.column + (closing ? 0
                        : 1);
                } else {
