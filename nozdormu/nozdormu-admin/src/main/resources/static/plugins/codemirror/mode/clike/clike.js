...
(function (mod) {
    ...
    CodeMirror.defineMode("clike", function (config, parserConfig) {
        ...
        function maybeEOL(stream, state) {
            if (parserConfig.typeFirstDefinitions && stream.eol() && isTopScope(
                state.context)) {
        ...
        return {
            ...
            token: function (stream, state) {
                ...
                if (style == "variable" &&
                    ((state.prevToken == "def" ||
                        (parserConfig.typeFirstDefinitions && typeBefore(stream,
                            state, stream.start) &&
            ...
            indent: function (state, textAfter) {
                ...
                return ctx.indented + (closing ? 0 : indentUnit) +
                    (!closing && switchBlock && !/^(?:case|default)\b/.test(
                        textAfter) ? indentUnit : 0);
            },
    ...
    def(["text/x-c++src", "text/x-c++hdr"], {
        name: "clike",
        keywords: words(cKeywords
            + " dynamic_cast namespace reinterpret_cast try explicit new " +
