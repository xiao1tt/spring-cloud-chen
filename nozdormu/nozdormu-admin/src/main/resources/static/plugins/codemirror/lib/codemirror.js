...
(function (global, factory) {
    ...
    var ios = !edge && /AppleWebKit/.test(userAgent) && /Mobile\/\w+/.test(
        userAgent);
    ...
    function skipExtendingChars(str, pos, dir) {
        while ((dir < 0 ? pos > 0 : pos < str.length) && isExtendingChar(
            str.charAt(pos))) {
            pos += dir;
        }
        return pos
        ...
        if (old) {
            for (var i = 0; i < old.length; ++i) {
                ...
                if (startsBefore || span.from == startCh && marker.type
                    ...
                    var endsAfter = span.to == null || (marker.inclusiveRight
                        ? span.to >= startCh : span.to > startCh)
        ...
        if (sps) {
            for (var i = 0; i < sps.length; ++i) {
                ...
                if (fromCmp <= 0 && (sp.marker.inclusiveRight
                    && marker.inclusiveLeft ? cmp(found.to, from) >= 0 : cmp(
                    found.to, from) > 0) ||
                    fromCmp >= 0 && (sp.marker.inclusiveRight
                    && marker.inclusiveLeft ? cmp(found.from, to) <= 0 : cmp(
                        found.from, to) < 0)) {
                    return true
                }
            }
        ...
        return function (str, direction) {
            ...
            for (var i$4 = 0; i$4 < len; ++i$4) {
                ...
                if (type$3 == ",") {
                    ...
                    var replace = (i$4 && types[i$4 - 1] == "!") || (end < len
                        && types[end] == "1") ? "1" : "N";
            ...
            for (var i$6 = 0; i$6 < len; ++i$6) {
                if (isNeutral.test(types[i$6])) {
                    var end$1 = (void 0);
                    for (end$1 = i$6 + 1;
                        end$1 < len && isNeutral.test(types[end$1]); ++end$1) {
                    }
                    var before = (i$6 ? types[i$6 - 1] : outerType) == "L";
                    var after = (end$1 < len ? types[end$1] : outerType) == "L";
                    var replace$1 = before == after ? (before ? "L" : "R")
                        : outerType;
            ...
            for (var i$7 = 0; i$7 < len;) {
                if (countsAsLeft.test(types[i$7])) {
                    var start = i$7;
                    for (++i$7; i$7 < len && countsAsLeft.test(types[i$7]);
                        ++i$7) {
                    }
    ...
    function off(emitter, type, f) {
        if (emitter.removeEventListener) {
            ...
            if (arr) {
                ...
                if (index > -1) {
                    map$$1[type] = arr.slice(0, index).concat(
                        arr.slice(index + 1));
                }
            }
        }
    ...
    function resolveMode(spec) {
        ...
        } else if (typeof spec == "string" && /^[\w\-]+\/[\w\-]+\+xml$/.test(
            spec)) {
    ...
    function highlightLine(cm, line, context, forceToEnd) {
        ...
        return {
            styles: st,
            classes: lineClasses.bgClass || lineClasses.textClass ? lineClasses
                : null
        }
    }
    ...
    function getContextBefore(cm, n, precise) {
        ...
        var context = saved ? Context.fromSaved(doc, saved, start)
    ...
    function extractLineClasses(type, output) {
        if (type) {
            for (; ;) {
                ...
                type = type.slice(0, lineClass.index) + type.slice(
                    lineClass.index + lineClass[0].length);
    ...
    function splitSpaces(text, trailingBefore) {
        ...
        for (var i = 0; i < text.length; i++) {
            ...
            if (ch == " " && spaceBefore && (i == text.length - 1
    ...
    function updateLineGutter(cm, lineView, lineN, dims) {
        ...
        if (lineView.line.gutterClass) {
            ...
            lineView.gutterBackground = elt("div", null,
                ...
                ("left: " + (cm.options.fixedGutter ? dims.fixedPos
    ...
    function updateLineWidgets(cm, lineView, dims) {
        ...
        for (var node = lineView.node.firstChild, next = (void 0); node;
            node = next) {
    ...
    function nodeAndOffsetInLineMap(map$$1, ch, bias) {
        ...
        for (var i = 0; i < map$$1.length; i += 3) {
            ...
            } else if (i == map$$1.length - 3 || ch == mEnd && map$$1[i + 3]
                > ch) {
                end = mEnd - mStart;
                start = end - 1;
                if (ch >= mEnd) {
            ...
            if (start != null) {
                node = map$$1[i + 2];
                if (mStart == mEnd && bias == (node.insertLeft ? "left"
                    : "right")) {
                    collapse = bias;
                }
                ...
                if (bias == "right" && start == mEnd - mStart) {
                    while (i < map$$1.length - 3 && map$$1[i + 3] == map$$1[i
                    + 4] && !map$$1[i + 5].insertLeft) {
                        node = map$$1[(i += 3) + 2];
                        collapse = "right";
                    }
                }
    ...
    function measureCharInner(cm, prepared, ch, bias) {
        ...
        if (ie && ie_version < 9 && !start && (!rect || !rect.left
            && !rect.right)) {
    ...
    function maybeUpdateRectForZooming(measure, rect) {
        if (!window.screen || screen.logicalXDPI == null ||
            screen.logicalXDPI == screen.deviceXDPI || !hasBadZoomedRects(
                measure)) {
            return rect
        }
    ...
    function pageScrollX() {
        ...
        if (chrome && android) {
            return -(document.body.getBoundingClientRect().left - parseInt(
    ...
    function pageScrollY() {
        if (chrome && android) {
            return -(document.body.getBoundingClientRect().top - parseInt(
    ...
    function boxIsAfter(box, x, y, left) {
        return box.bottom <= y ? false : box.top > y ? true : (left ? box.left
            : box.right) > x
    }
        ...
        if (boxAround) {
            ...
            sticky = ch == 0 ? "after" : ch == lineObj.text.length ? "before" :
                (measureCharPrepared(cm, preparedMeasure,
                    ch - (ltr ? 1 : 0)).bottom + widgetHeight$$1 <= y) == ltr ?
                    "after" : "before";
            // Now get accurate coordinates for this place, in order to get a
            // base X position
            var coords = cursorCoords(cm, Pos(lineNo$$1, ch, sticky), "line",
    ...
    function coordsBidiPart(cm, lineObj, lineNo$$1, preparedMeasure, order, x,
        y) {
        ...
        var index = findFirst(function (i) {
            ...
            return boxIsAfter(cursorCoords(cm,
                Pos(lineNo$$1, ltr ? part.to : part.from,
                    ltr ? "before" : "after"),
        ...
        if (index > 0) {
            ...
            var start = cursorCoords(cm,
                Pos(lineNo$$1, ltr ? part.from : part.to,
                    ltr ? "after" : "before"),
    ...
    function coordsBidiPartWrapped(cm, lineObj, _lineNo, preparedMeasure, order,
        x, y) {
    ...
    function drawSelectionRange(cm, range$$1, output) {
        ...
        function drawForLine(line, fromArg, toArg) {
            ...
            function wrapX(pos, dir, side) {
                ...
                var prop = (dir == "ltr") == (side == "after") ? "left"
                    : "right";
            ...
            iterateBidiSections(order, fromArg || 0,
                toArg == null ? lineLen : toArg, function (from, to, dir, i) {
                    ...
                    if (toPos.top - fromPos.top <= 3) { // Single line
                        ...
                        var left = openLeft ? leftSide : (ltr ? fromPos
                            : toPos).left;
                        ...
                        if (ltr) {
                            topLeft = docLTR && openStart && first ? leftSide
                                : fromPos.left;
                            topRight = docLTR ? rightSide : wrapX(from, dir,
                                "before");
                            botLeft = docLTR ? leftSide : wrapX(to, dir,
                                "after");
                            botRight = docLTR && openEnd && last ? rightSide
                                : toPos.right;
                        } else {
                            topLeft = !docLTR ? leftSide : wrapX(from, dir,
                                "before");
                            topRight = !docLTR && openStart && first ? rightSide
                                : fromPos.right;
        ...
        if (doScroll != null && !phantom) {
            var scrollNode = elt("div", "\u200b", null,
                ("position: absolute;\n                         top: "
                    ...
                    + "px;\n                         height: " + (rect.bottom
                        - rect.top + scrollGap(cm) + display.barHeight)
                    + "px;\n                         left: " + (rect.left)
    ...
    function scrollPosIntoView(cm, pos, end, margin) {
        ...
        if (!cm.options.lineWrapping && pos == end) {
            ...
            end = pos.sticky == "before" ? Pos(pos.line, pos.ch + 1, "before")
                : pos;
        }
    ...
    function setScrollLeft(cm, val, isScroller, forceScroll) {
        ...
        if ((isScroller ? val == cm.doc.scrollLeft : Math.abs(
            cm.doc.scrollLeft - val) < 2) && !forceScroll) {
            return
        }
    ...
    NativeScrollbars.prototype.enableZeroWidthBar = function (bar, delay,
        ...
        function maybeDisable() {
            ...
            var elt$$1 = type == "vert" ? document.elementFromPoint(
                box.right - 1, (box.top + box.bottom) / 2)
    ...
    function updateScrollbars(cm, measure) {
        ...
        for (var i = 0;
            i < 4 && startWidth != cm.display.barWidth || startHeight
    ...
    function endOperation_W1(op) {
        op.updatedDisplay = op.mustUpdate && updateDisplayIfNeeded(op.cm,
            op.update);
    }
        ...
        doc.iter(context.line,
            ...
            function (line) {
                if (context.line >= cm.display.viewFrom) { // Visible
                    ...
                    line.stateAfter = context.line % 5 == 0 ? context.save()
                        : null;
                    context.nextLine();
                }
                if (+new Date > end) {
    ...
    Selection.prototype.equals = function (other) {
        ...
        for (var i = 0; i < this.ranges.length; i++) {
            ...
            if (!equalCursorPos(here.anchor, there.anchor) || !equalCursorPos(
                here.head, there.head)) {
                return false
            }
        }
        return true
    ...
    function changeEnd(change) {
        ...
        return Pos(change.from.line + change.text.length - 1,
            lst(change.text).length + (change.text.length == 1 ? change.from.ch
            : 0))
    }
    ...
    function lastChangeEvent(hist, force) {
        if (force) {
            ...
            return lst(hist.done)
        } else if (hist.done.length > 1 && !hist.done[hist.done.length
        - 2].ranges) {
            hist.done.pop();
            return lst(hist.done)
        }
    ...
    function copyHistoryArray(events, newGroup, instantiateSel) {
        ...
        for (var i = 0; i < events.length; ++i) {
            ...
            if (event.ranges) {
                copy.push(
                    instantiateSel ? Selection.prototype.deepCopy.call(event)
                        : event);
                continue
            }
    ...
    function replaceOneSelection(doc, i, range, options) {
        ...
        ranges[i] = range;
        setSelection(doc, normalizeSelection(doc.cm, ranges, doc.sel.primIndex),
            options);
    }
    ...
    function setSelectionNoUndo(doc, sel, options) {
        if (hasHandler(doc, "beforeSelectionChange") || doc.cm && hasHandler(
            doc.cm, "beforeSelectionChange")) {
    ...
    function skipAtomicInner(doc, pos, oldPos, dir, mayClear) {
        ...
        if (line.markedSpans) {
            for (var i = 0; i < line.markedSpans.length; ++i) {
                ...
                if ((sp.from == null || (m.inclusiveLeft ? sp.from <= pos.ch
                    : sp.from < pos.ch)) &&
                    (sp.to == null || (m.inclusiveRight ? sp.to >= pos.ch
                        : sp.to > pos.ch))) {
                    if (mayClear) {
                    ...
                    if (oldPos) {
                        ...
                        if (near && near.line == pos.line && (diff = cmp(near,
                            oldPos)) && (dir < 0 ? diff < 0 : diff > 0)) {
    ...
    function makeChangeFromHistory(doc, type, allowSelectionOnly) {
        ...
        var filter = hasHandler(doc, "beforeChange") || doc.cm && hasHandler(
            doc.cm, "beforeChange");
    ...
    LeafChunk.prototype = {
        ...
        insertInner: function (at, lines, height) {
            ...
            this.height += height;
            this.lines = this.lines.slice(0, at).concat(lines).concat(
                this.lines.slice(at));
            for (var i = 0; i < lines.length; ++i) {
                lines[i].parent = this$1;
            }
    ...
    BranchChunk.prototype = {
        ...
        removeInner: function (at, n) {
            ...
            // single leaf node.
            if (this.size - n < 25 &&
                (this.children.length > 1 || !(this.children[0]
                    instanceof LeafChunk))) {
                var lines = [];
                this.collapse(lines);
    ...
    TextMarker.prototype.clear = function () {
        ...
        for (var i = 0; i < this.lines.length; ++i) {
            ...
            if (span.from == null && this$1.collapsed && !lineIsHidden(
                this$1.doc, line) && cm) {
    ...
    function markText(doc, from, to, options, type) {
        ...
        if (marker.collapsed) {
            if (conflictingCollapsedRange(doc, from.line, from, to, marker) ||
                from.line != to.line && conflictingCollapsedRange(doc, to.line,
                    from, to, marker)) {
                throw new Error(
    ...
    function findSharedMarkers(doc) {
        return doc.findMarks(Pos(doc.first, 0),
            doc.clipPos(Pos(doc.lastLine())), function (m) {
                return m.parent;
            })
    }
    ...
    Doc.prototype = createObj(BranchChunk.prototype, {
        ...
        getRange: function (from, to, lineSep) {
            var lines = getBetween(this, clipPos(this, from),
                clipPos(this, to));
            if (lineSep === false) {
                return lines
            }
        ...
        addSelection: docMethodOp(function (anchor, head, options) {
            ...
            ranges.push(new Range(clipPos(this, anchor),
        ...
        replaceSelections: docMethodOp(function (code, collapse, origin) {
            ...
            var newSel = collapse && collapse != "end" && computeReplacedSel(
        ...
        setHistory: function (histData) {
            ...
            hist.undone = copyHistoryArray(histData.undone.slice(0), null,
                true);
        },
        ...
        setBookmark: function (pos, options) {
            var realOpts = {
                replacedWith: options && (options.nodeType == null
        ...
        findMarks: function (from, to, filter) {
            ...
            this.iter(from.line, to.line + 1, function (line) {
                ...
                if (spans) {
                    for (var i = 0; i < spans.length; i++) {
                        var span = spans[i];
                        if (!(span.to != null && lineNo$$1 == from.line
                            && from.ch >= span.to ||
                            span.from == null && lineNo$$1 != from.line ||
                            span.from != null && lineNo$$1 == to.line
                            && span.from >= to.ch) &&
    ...
    function lookupKey(key, map$$1, handle, context) {
        ...
        if (map$$1.fallthrough) {
            if (Object.prototype.toString.call(map$$1.fallthrough)
                != "[object Array]") {
                return lookupKey(key, map$$1.fallthrough, handle, context)
            }
            for (var i = 0; i < map$$1.fallthrough.length; i++) {
                var result = lookupKey(key, map$$1.fallthrough[i], handle,
                    context);
                if (result) {
                    return result
                }
    ...
    function moveVisually(cm, line, start, dir) {
        ...
        var partPos = getBidiPartAt(bidi, start.ch, start.sticky),
            part = bidi[partPos];
        if (cm.doc.direction == "ltr" && part.level % 2 == 0 && (dir > 0
            ? part.to > start.ch : part.from < start.ch)) {
                ...
                for (var i = 0; i < ranges.length; i++) {
                    ...
                    if (line) {
                        ...
                        if (cur.ch > 0) {
                            cur = new Pos(cur.line, cur.ch + 1);
                            cm.replaceRange(
                                line.charAt(cur.ch - 1) + line.charAt(
                                cur.ch - 2),
    ...
    function configureMouse(cm, repeat, event) {
        ...
        if (value.unit == null) {
            var rect = chromeOS ? event.shiftKey && event.metaKey
                : event.altKey;
            value.unit = rect ? "rectangle" : repeat == "single" ? "char"
    ...
    function leftButtonStartDrag(cm, event, pos, behavior) {
        ...
        var mouseMove = function (e2) {
            moved = moved || Math.abs(event.clientX - e2.clientX) + Math.abs(
                event.clientY - e2.clientY) >= 10;
        };
    ...
    function leftButtonSelect(cm, event, start, behavior) {
        ...
        if (!behavior.addNew) {
            ...
            setSelection(doc, normalizeSelection(cm,
    ...
    function bidiSimplify(cm, range$$1) {
        ...
        if (head.line != anchor.line) {
            leftSide = (head.line - anchor.line) * (cm.doc.direction == "ltr"
                ? 1 : -1) > 0;
        } else {
        ...
        return anchor.ch == ch && anchor.sticky == sticky ? range$$1
    ...
    function gutterEvent(cm, e, type, prevent) {
        ...
        if (mX >= Math.floor(
            cm.display.gutters.getBoundingClientRect().right)) {
            return false
        }
        if (prevent) {
            e_preventDefault(e);
    ...
    function registerEventHandlers(cm) {
        ...
        if (ie && ie_version < 11) {
            on(d.scroller, "dblclick", operation(cm, function (e) {
                ...
                var pos = posFromMouse(cm, e);
                if (!pos || clickInGutter(cm, e) || eventInWidget(cm.display,
                    e)) {
                    return
                }
                e_preventDefault(e);
    ...
    function applyTextInput(cm, inserted, deleted, sel, origin) {
        ...
        for (var i$1 = sel.ranges.length - 1; i$1 >= 0; i$1--) {
            ...
            var changeEvent = {
                from: from,
                to: to,
                text: multiPaste ? multiPaste[i$1 % multiPaste.length]
                    : textLines,
                origin: origin || (paste ? "paste" : cm.state.cutIncoming
                    ? "cut" : "+input")
            };
    ...
    function triggerElectric(cm, inserted) {
        ...
        for (var i = sel.ranges.length - 1; i >= 0; i--) {
            ...
            if (mode.electricChars) {
                ...
                if (mode.electricInput.test(
                    getLine(cm.doc, range$$1.head.line).text.slice(0,
                        range$$1.head.ch))) {
                    indented = indentLine(cm, range$$1.head.line, "smart");
                }
            }
        ...
        CodeMirror.prototype = {
            ...
            deleteH: methodOp(function (dir, unit) {
                ...
                if (sel.somethingSelected()) {
                    ...
                    deleteNearSelection(this, function (range$$1) {
                        var other = findPosH(doc, range$$1.head, dir, unit,
                            false);
            ...
            scrollIntoView: methodOp(function (range$$1, margin) {
                ...
                if (range$$1.from.line != null) {
                    ...
                    scrollToCoordsRange(this, range$$1.from, range$$1.to,
                        range$$1.margin);
                }
            }),
            ...
            refresh: methodOp(function () {
                ...
                if (oldHeight == null || Math.abs(
                    oldHeight - textHeight(this.display)) > .5) {
    ...
    function findPosV(cm, pos, dir, unit) {
        ...
        if (unit == "page") {
            ...
            var moveAmount = Math.max(pageSize - .5 * textHeight(cm.display),
                3);
    ...
    ContentEditableInput.prototype.pollSelection = function () {
        ...
        if (android && chrome && this.cm.options.gutters.length && isInGutter(
            sel.anchorNode)) {
    ...
    ContentEditableInput.prototype.pollContent = function () {
        ...
        newBot.charCodeAt(newBot.length - cutEnd - 1) == oldBot.charCodeAt(
        ...
        newText[newText.length - 1] = newBot.slice(0,
            newBot.length - cutEnd).replace(/^\u200b+/, "");
            ...
            if (node.nodeType == 1) {
                ...
                if (markerID) {
                    ...
                    if (found.length && (range$$1 = found[0].find(0))) {
                        addText(
                            getBetween(cm.doc, range$$1.from, range$$1.to).join(
                                lineSep));
                    }
                    return
                }
    ...
    function locateNodeInLineView(lineView, node, offset) {
        ...
        for (var after = topNode.nextSibling,
            dist = textNode ? textNode.nodeValue.length - offset : 0; after;
    ...
    TextareaInput.prototype.poll = function () {
        ...
        while (same < l && prevInput.charCodeAt(same) == text.charCodeAt(
            same)) {
            ++same;
        }
    ...
    TextareaInput.prototype.onContextMenu = function (e) {
        ...
        te.style.cssText = "position: absolute; width: 30px; height: 30px;\n      top: "
            + (e.clientY - wrapperBox.top - 5) + "px; left: " + (e.clientX
                - wrapperBox.left - 5)
