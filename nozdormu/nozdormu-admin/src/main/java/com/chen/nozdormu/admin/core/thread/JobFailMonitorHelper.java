...
public class JobFailMonitorHelper {
    ...
    public void start() {
        monitorThread = new Thread(() -> {
            ...
            while (!toStop) {
                try {
                    List<Long> failLogIds = JobAdminConfig.getAdminConfig().getJobLogDao()
                            .findFailJobLogIds(1000);
                    if (failLogIds != null && !failLogIds.isEmpty()) {
                        for (long failLogId : failLogIds) {

                            // lock log
                            int lockRet = JobAdminConfig
                                    .getAdminConfig().getJobLogDao().updateAlarmStatus(failLogId, 0, -1);
                            if (lockRet < 1) {
                                continue;
                            }
                            JobLog log = JobAdminConfig.getAdminConfig().getJobLogDao().load(failLogId);
                            JobInfo info = JobAdminConfig.getAdminConfig().getJobInfoDao()
                            ...
                            if (log.getExecutorFailRetryCount() > 0) {
                                ...
                                String retryMsg = "<br><br><span style=\"color:#F39C12;\" > >>>>>>>>>>>" + I18nUtil
                                        .getString("jobconf_trigger_type_retry") + "<<<<<<<<<<< </span><br>";
                            ...
                            if (info != null && info.getAlarmEmail() != null
                                ...
                                boolean alarmResult = JobAdminConfig
                                        .getAdminConfig().getJobAlarmer().alarm(info, log);
                                newAlarmStatus = alarmResult ? 2 : 3;
                            } else {
                                newAlarmStatus = 1;
                            }

                            JobAdminConfig.getAdminConfig().getJobLogDao()
        ...
        monitorThread.setName("  admin JobFailMonitorHelper");
        monitorThread.start();
    }
